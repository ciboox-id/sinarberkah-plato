            ...
            portfolio.map((data) => (
              <div key={data._id} className="xl:w-1/4 md:w-1/2 p-4">
                <div className="bg-offwhite p-6 rounded-lg">
                  <img className="h-40 rounded w-full object-cover object-center mb-6" src={data.image.image_url} alt={data.title} />
                  <h3 className="tracking-widest text-main-color text-xs font-bold title-font">{data.type}</h3>
                  <h2 className="text-lg text-dark-main font-semibold title-font mb-4">{data.title}</h2>
