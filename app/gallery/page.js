    ...
    <section className="px-14">
      <div className="container py-24 mx-auto">
        <div className="flex flex-col text-center w-full mb-20">
          <h1 className="sm:text-4xl text-3xl font-bold title-font mb-4 text-dark-main">Gallery Pola Sinar Berkah</h1>
          ...
          gallery.map((data) => (
            <div key={data._id} className="lg:w-1/3 sm:w-1/2 p-4">
              <div className="flex relative">
                <img className="absolute inset-0 w-full h-full object-cover object-center" src={data.image.image_url} alt={data.title} />
                <div className="px-8 py-10 relative z-10 w-full border-4 border-gray-200 bg-white opacity-0 hover:opacity-100 hover:bg-offwhite">
                  <h2 className="tracking-widest text-sm title-font font-bold text-main-color mb-1">{data.type}</h2>
                  <h1 className="title-font text-lg font-semibold font-mulish text-gray-900 mb-3">{data.title}</h1>
